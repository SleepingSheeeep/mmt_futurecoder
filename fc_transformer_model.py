...
from mmt_future_context.fc_transformer_base import (
    FutureContextTransformerModelBase,
)


@register_model("futurecontext_transformer")
class FutureContextTransformerModel(FutureContextTransformerModelBase):
    ...
    def __init__(self, args, encoder, decoder, futurecontex):
        cfg = TransformerConfig.from_namespace(args)
        super().__init__(cfg, encoder, decoder, futurecontex)
        self.args = args
    ...
    @classmethod
    def build_futurecoder(cls, args, tgt_dict, embed_tokens):
        return  super().build_futurecoder(
            TransformerConfig.from_namespace(args), tgt_dict, embed_tokens
        )


# architectures
@register_model_architecture("futurecontext_transformer", "futurecontext_transformer_arch")
def base_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
    # args.futurecoder_layers = getattr(args, "futurecoder_layers", 1)
